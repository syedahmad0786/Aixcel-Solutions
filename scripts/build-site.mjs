...
function policyBody(page) {
  ...
  const content = privacy ? `<div class="prose">
    ...
    <h2>Systems Desk and AI processing</h2><p>When you submit a Systems Desk question, the current problem brief, recent browser-held conversation context, and relevant approved Aixcel evidence are sent through Aixcel's server to a selected free model on OpenRouter to produce an answer. Do not submit passwords, credentials, health information, payment data, or confidential client records. The initial release saves the problem brief but does not save the chat transcript as account history.</p>
    <h2>Service providers and international processing</h2><p>Vercel hosts the website and server function, Supabase provides authentication and database services, OpenRouter routes Systems Desk model requests, Cal.com provides booking, and Google may deliver website font files. Email, communications, analytics if enabled, and project tools may also process information on Aixcel's behalf. Those providers may operate in other countries and apply their own infrastructure, terms, and contractual safeguards.</p>
